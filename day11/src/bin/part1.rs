...
impl Monkey {
    fn inspect(&mut self) -> Vec<(usize, i64)> {
        let mut ret: Vec<(usize, i64)> = Vec::new();
        for i in &self.items {
            self.inspect += 1;
            let operand:i64 = match self.operation.1 {
                Operand::Num(n) => n.into(),
                Operand::Old => *i,
            };
            let lvl = match self.operation.0 {
                '+' => i + operand,
                '-' => i - operand,
                '*' => i * operand,
                '/' => i / operand,
            ...
            if lvl % i64::from(self.test.0) == 0 {
                ret.push((self.test.1, lvl));
            } else {
