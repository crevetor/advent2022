...
impl FileSystem {
    ...
    fn get_dirs_bigger_than(&self, size: usize, startidx: usize) -> Vec<usize> {
        let mut ret: Vec<usize> = Vec::new();
        for idx in &self.contents[startidx].contents {
            let dirsize = self.size(*idx);
            if self.contents[*idx].is_dir() {
                ret.append(&mut self.get_dirs_bigger_than(size, *idx));
            }
            if dirsize > size {
                ret.push(dirsize);
            }
...
fn main() {
    ...
    let rootidx = fs.find_dir("/", None).unwrap();

    let rootsize = fs.size(rootidx);
    let current_free = 70000000 - rootsize;
    let need_to_free = 30000000 - current_free;

    let sizes = fs.get_dirs_bigger_than(need_to_free, rootidx);
    println!("{}", sizes.iter().map(|x| x.to_string()).collect::<Vec<String>>().join(","));
    println!("Freeable : {}", sizes.iter().filter(|x| **x != 0).min().unwrap());
}